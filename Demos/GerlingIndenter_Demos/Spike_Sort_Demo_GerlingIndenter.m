...
dataName = 'GerlingIndenter_DemoData_trial1'; % add file Name!
...
[Spike_Train, Spike_Shape] = Spike_Bandpass_SNP(All_short, 1500, 100, 48); % ENTER PEAK BANDPASS
...
idx = DBSCAN(PCA_weights(:,1:2),0.003,3); % (PCA1 and PCA2, epsilon, minimum spikes per cluster)  
...
[Output_R, Output_F, Output_D, Output_R_boolean] = Reduce_Spike_Train(All_short, Show_Spike_Train, 20000);
Summary_Plot(Output_R, Output_F, Output_D, Output_R_boolean, dataName, Cluster_Num);
...
Disp_points = Displacement_inflections(Output_D, 1, 10000, 60000, 80000);
%  -------- Step 6-3: Generate ISI file
[IFF_out, IFF_no_zero_out, rISI_ALL, rISI_DETAIL] = Data_analysis(Output_R_boolean, Disp_points, 20000);
[ALL_ISI_OF,StimPhase_ISI_OF,Dynamic_ISI_OF,Late_Static_ISI_OF, End_ISI_OF] = ISI_output_file(Mags_D, Mags_F, rISI_ALL);
